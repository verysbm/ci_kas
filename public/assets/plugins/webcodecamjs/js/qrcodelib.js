    ...
    for (var y = 0; y < dimension; y++) {
        ...
        try {
            for (var x = 0; x < max; x += 2) {
                ...
                var bit = image[Math.floor(points[x]) + qrcode.width * Math.floor(points[x +
                    1])];
...
function PerspectiveTransform(a11, a21, a31, a12, a22, a32, a13, a23, a33) {
    ...
    this.buildAdjoint = function() {
        return new PerspectiveTransform(this.a22 * this.a33 - this.a23 * this.a32, this.a23 * this.a31 - this.a21 * this.a33, this.a21 * this.a32 -
            this.a22 * this.a31, this.a13 * this.a32 - this.a12 * this.a33, this.a11 * this.a33 - this.a13 * this.a31, this.a12 * this.a31 - this.a11 * this.a32, this.a12 * this.a23 - this.a13 * this.a22, this.a13 * this.a21 - this.a11 * this.a23, this.a11 * this.a22 - this.a12 * this.a21)
    };
    this.times = function(other) {
        return new PerspectiveTransform(this.a11 * other.a11 + this.a21 * other.a12 + this.a31 * other.a13, this.a11 * other.a21 + this.a21 * other.a22 + this.a31 * other.a23, this.a11 * other.a31 + this.a21 * other.a32 + this.a31 * other.a33, this.a12 * other.a11 + this.a22 * other.a12 +
...
function Detector(image) {
    ...
    this.calculateModuleSize =
        function(topLeft, topRight, bottomLeft) {
    ...
    this.computeDimension = function(topLeft, topRight, bottomLeft, moduleSize) {
        ...
        var tlblCentersDimension = Math.round(this.distance(topLeft, bottomLeft) /
            moduleSize);
    ...
    this.findAlignmentInRegion = function(overallEstModuleSize, estAlignmentX, estAlignmentY, allowanceFactor) {
        ...
        if (alignmentAreaRightX -
            alignmentAreaLeftX < overallEstModuleSize * 3) throw "Error";
        var alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
        var alignmentAreaBottomY = Math.min(qrcode.height - 1, estAlignmentY + allowance);
        var alignmentFinder = new AlignmentPatternFinder(this.image, alignmentAreaLeftX, alignmentAreaTopY, alignmentAreaRightX - alignmentAreaLeftX, alignmentAreaBottomY - alignmentAreaTopY, overallEstModuleSize, this.resultPointCallback);
        return alignmentFinder.find()
    };
    this.createTransform = function(topLeft, topRight, bottomLeft, alignmentPattern,
        dimension) {
        ...
        var transform = PerspectiveTransform.quadrilateralToQuadrilateral(3.5, 3.5, dimMinusThree,
            3.5, sourceBottomRightX, sourceBottomRightY, 3.5, dimMinusThree, topLeft.X, topLeft.Y, topRight.X, topRight.Y, bottomRightX, bottomRightY, bottomLeft.X, bottomLeft.Y);
        return transform
    };
    ...
    this.processFinderPatternInfo = function(info) {
        ...
        if (provisionalVersion.AlignmentPatternCenters.length > 0) {
            ...
            var estAlignmentX =
                Math.floor(topLeft.X + correctionToTopLeft * (bottomRightX - topLeft.X));
...
var FORMAT_INFO_DECODE_LOOKUP = new Array(new Array(21522, 0), new Array(20773, 1), new Array(24188, 2), new Array(23371, 3), new Array(17913, 4), new Array(16590, 5), new Array(20375, 6), new Array(19104, 7), new Array(30660, 8), new Array(29427, 9), new Array(32170, 10), new Array(30877, 11), new Array(26159, 12), new Array(25368, 13), new Array(27713, 14), new Array(26998, 15), new Array(5769, 16), new Array(5054, 17), new Array(7399, 18), new Array(6608, 19), new Array(1890, 20), new Array(597, 21), new Array(3340, 22), new Array(2107, 23),
    new Array(13663, 24), new Array(12392, 25), new Array(16177, 26), new Array(14854, 27), new Array(9396, 28), new Array(8579, 29), new Array(11994, 30), new Array(11245, 31));
...
FormatInformation.doDecodeFormatInformation = function(maskedFormatInfo) {
    ...
    if (bestDifference <=
        3) return new FormatInformation(bestFormatInfo);
    return null
};
...
function BitMatrix(width, height) {
    ...
    this.setRegion = function(left, top, width, height) {
        ...
        if (height <
            1 || width < 1) throw "Height and width must be at least 1";
...
function BitMatrixParser(bitMatrix) {
    ...
    this.readVersion = function() {
        ...
        if (this.parsedVersion !=
            null && this.parsedVersion.DimensionForVersion == dimension) return this.parsedVersion;
...
function ReedSolomonDecoder(field) {
    ...
    this.decode = function(received, twoS) {
        ...
        if (noError) return;
        var syndrome = new GF256Poly(this.field,
    ...
    this.findErrorMagnitudes = function(errorEvaluator,
        errorLocations, dataMatrix) {
...
function GF256Poly(field, coefficients) {
    ...
    this.multiply1 = function(other) {
        ...
        for (var i = 0; i < aLength; i++) {
            ...
            for (var j = 0; j < bLength; j++) product[i + j] = GF256.addOrSubtract(product[i + j], this.field.multiply(aCoeff,
                bCoefficients[j]))
        }
        ...
        for (var ax = 0; ax < numSqrtArea; ax++) {
            ...
            for (var dy = 0; dy < areaHeight; dy++)
                for (var dx = 0; dx < areaWidth; dx++) {
                    var target = image[areaWidth *
                        ax + dx + (areaHeight * ay + dy) * qrcode.width];
...
qrcode.grayScaleToBitmap = function(grayScale) {
    ...
    for (var ay = 0; ay < sqrtNumArea; ay++)
        for (var ax = 0; ax < sqrtNumArea; ax++)
            for (var dy = 0; dy < areaHeight; dy++)
                for (var dx = 0; dx < areaWidth; dx++) bitmap[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] = grayScale[areaWidth * ax + dx + (areaHeight *
                    ay + dy) * qrcode.width] < middle[ax][ay] ? true : false;
    return bitmap
};
...
function FinderPattern(posX, posY, estimatedModuleSize) {
    ...
    this.aboutEquals = function(moduleSize, i, j) {
        if (Math.abs(i - this.y) <=
            moduleSize && Math.abs(j - this.x) <= moduleSize) {
...
function FinderPatternFinder() {
    ...
    this.foundPatternCross = function(stateCount) {
        ...
        return Math.abs(moduleSize - (stateCount[0] << INTEGER_MATH_SHIFT)) < maxVariance && Math.abs(moduleSize - (stateCount[1] << INTEGER_MATH_SHIFT)) < maxVariance && Math.abs(3 * moduleSize - (stateCount[2] << INTEGER_MATH_SHIFT)) < 3 * maxVariance && Math.abs(moduleSize - (stateCount[3] << INTEGER_MATH_SHIFT)) < maxVariance && Math.abs(moduleSize -
    ...
    this.crossCheckVertical = function(startI, centerJ, maxCount, originalStateCountTotal) {
        ...
        if (i <
            0 || stateCount[1] > maxCount) return NaN;
        ...
        if (!isNaN(centerI)) {
            ...
            if (!isNaN(centerJ)) {
                ...
                for (var index = 0; index < max; index++) {
                    ...
                    if (center.aboutEquals(estimatedModuleSize,
                            centerI, centerJ)) {
    ...
    this.selectBestPatterns = function() {
        ...
        if (startSize > 3) {
            ...
            for (var i = 0; i < startSize; i++) {
                var centerValue =
                    this.possibleCenters[i].EstimatedModuleSize;
            ...
            for (var i = 0; i < this.possibleCenters.length && this.possibleCenters.length >
                3; i++) {
...
function AlignmentPattern(posX, posY, estimatedModuleSize) {
    ...
    this.aboutEquals = function(moduleSize,
        i, j) {
...
function QRCodeDataBlockReader(blocks, version, numErrorCorrectionCode) {
    ...
    this.getNextBits = function(numBits) {
        ...
        if (numBits < this.bitPointer + 1) {
            ...
            bits =
                (this.blocks[this.blockPointer] & mask) >> this.bitPointer - numBits + 1;
            ...
            return bits
        } else if (numBits < this.bitPointer +
            1 + 16) {
            var mask1 = 0;
            var mask3 = 0;
            for (var i = 0; i < this.bitPointer + 1; i++) mask1 += 1 << i;
            ...
            bits = bitsFirstBlock +
                bitsSecondBlock + bitsThirdBlock;
    ...
    this.__defineGetter__("DataByte", function() {
        ...
        do {
            ...
            if (mode != MODE_NUMBER && mode != MODE_ROMAN_AND_NUMBER && mode != MODE_8BIT_BYTE && mode != MODE_KANJI) throw "Invalid mode: " + mode + " in (block:" + this.blockPointer + " bit:" +
                this.bitPointer + ")";
